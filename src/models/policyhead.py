...
# compared with all one-hot word embeddings for all guess words (via dot prod.) to produce logits (masked for valid word indices),
...
class PolicyHead(nn.Module):
    ...
    def forward(self, h, valid_indices_batch, word_encodings): # word_encodings: [130, vocab_size]
        device = h.device
        
        query = self.linear(h)

        # Compute logits via dot product with all word one hot encodings
